/*
******************************************************************************
** Filename:  uab_SIERRA.c
**
** Description:
**       This file contains all of the functions for the monitoring/control
**       of the SIERRA stair lift system via the UAB module.
...
uint8_t                 UabSierraPlatformMounting;
uint8_t                 UabSierraPlatformPosition;
uint8_t                 UabSierraLeftsideRampPosition  = SIERRA_RAMP_UNKNOWN;
uint8_t                 UabSierraRightsideRampPosition = SIERRA_RAMP_UNKNOWN;
uint8_t                 UabSierraStartDownHoldoff = 0;
uint8_t                 UabSierraStartUpHoldoff   = 0;
...
uint8_t                 PlatformRampChangeDelay = 0;
...
void uabMajorFaultHandler(uint8_t cause)
{
    ...
    if (cause >= UAB_MAJOR_FAULT_RAMP_BOARD_INIT)
        cause -= 20;  /* For per minute Beep/CPU LED blink - use 10+ */
...
**               when:
**                      Sierra Current Sense is > 20.0 Amps (scaled)
**
...
#define SIERRA_CURR_SENSE_AMPS_MAX           25.0f
#define SIERRA_CURR_SENSE_SCALE_FACTOR_UP    0.94f   /* Max PWM 95% */
#define SIERRA_CURR_SENSE_SCALE_FACTOR_DOWN  0.80f   /* Max PWM 80% */
...
void uabSystemOverloadCheck(void)
{
    ...
    if (amps > 15.0)
    {
        curr_sense_amps = SIERRA_CURR_SENSE_AMPS_MAX; /* Non-scaled */
        if (UabUpDownLeftRightSwitchState == switch_up_left)
        {
            curr_sense_amps = SIERRA_CURR_SENSE_AMPS_MAX *
                              SIERRA_CURR_SENSE_SCALE_FACTOR_UP;
        }
        else
        {
            ...
            {
                curr_sense_amps = SIERRA_CURR_SENSE_AMPS_MAX *
                                  SIERRA_CURR_SENSE_SCALE_FACTOR_DOWN;
            }
        }
...
uabUpDownLeftRightSwitchState_t uabUpDownLeftRightSwitchStateGet(uint32_t pio0)
{
    ...
    *
    ** NOTE: The switches are swapped on associated PIO pins 
    ** between Tama and Pinnacle/Sierra!
    */
...
/*
******************************************************************************
** Function:     uabBrakeOff     (Pinnacle/Sierra Only)
**               uabBrakeOn
**
** Description:  These functions engages/releases the brake on the motor.
**
** Input  Parms: None
**
** Output Parms: None
...
** Output Parms: None
**
** Return Value: uint32_t  Current Sierra Safety Switch faults
**                           NO_FAULTS                0
**                           OB_UP_FAULT                    (1<<0)
**                           OB_DOWN_FAULT                  (1<<1)
**                           SIERRA_UPSIDE_RAMP_OB_FAULT    (1<<2)
**                           SIERRA_DOWNSIDE_RAMP_OB_FAULT  (1<<3)
**                           FR/PLATFORM_UNDER_FAULT        (1<<4)
**                           KEY_OFF_FAULT                  (1<<7)
**
******************************************************************************
*/
uint32_t uabFaultsGet(uint32_t pio0, uint32_t pio2)
{
    ...
    if ( (pio0 & SIERRA_UPSIDE_RAMP_OBSTR_SWITCH) ||
         ((prev_faults & SIERRA_UPSIDE_RAMP_OB_FAULT) && (!UabUpDownReleased)) )
        faults |= SIERRA_UPSIDE_RAMP_OB_FAULT;

    if ( (pio0 & SIERRA_DOWNSIDE_RAMP_OBSTR_SWITCH) ||
         ((prev_faults & SIERRA_DOWNSIDE_RAMP_OB_FAULT) && (!UabUpDownReleased)) )
        faults |= SIERRA_DOWNSIDE_RAMP_OB_FAULT;

    /* For Pinnacle/Sierra, if in Installation Mode, disregard indicated faults */
    if (UabInstMode)
    {
        faults &= ~(KEY_OFF_FAULT |
                    SIERRA_DOWNSIDE_RAMP_OB_FAULT | SIERRA_UPSIDE_RAMP_OB_FAULT |
                    FR_UNDER_FAULT);
    }
    ...
    if (UabUpDownLeftRightSwitchState == switch_up_left)
    {
        faults &= ~(SIERRA_DOWNSIDE_RAMP_OB_FAULT | OB_DOWN_FAULT | FR_UNDER_FAULT);
    }
    else
    {
        ...
        if (UabUpDownLeftRightSwitchState == switch_down_right)
        {
            faults &= ~(SIERRA_UPSIDE_RAMP_OB_FAULT | OB_UP_FAULT);
        }
    }

    if (UabSierraPlatformPosition == SIERRA_PLATFORM_FOLDED)
    {
    	 faults &= ~(SIERRA_DOWNSIDE_RAMP_OB_FAULT |
    	             SIERRA_UPSIDE_RAMP_OB_FAULT   |
                     FR_UNDER_FAULT);
    }
        ...
        for(i=0; i<8; i++)
        {
            ...
            if ( (prev_faults & bit) != (faults & bit) )
            {
                switch(bit)
                {
                    ...
                    case SIERRA_UPSIDE_RAMP_OB_FAULT:
                        if (faults & bit)
                          uabFaultSet(SIERRA_UPSIDE_RAMP_OB_FAULT,
                                      UPSIDE_RAMP_OB_SWITCH_NUM);
                        else
                          uabFaultClr(SIERRA_UPSIDE_RAMP_OB_FAULT,
                                      UPSIDE_RAMP_OB_SWITCH_NUM);
                        break;

                    case SIERRA_DOWNSIDE_RAMP_OB_FAULT:
                        if (faults & bit)
...
#define SIERRA_SOFT_START_DOWN_END_PWM       80
...
#define SIERRA_SLOW_CRUISE_UP_PWM            50
...
#define SIERRA_SLOW_CRUISE_DOWN_PWM          25
...
void uabMotionSet(uint32_t pio0, uint32_t pio2)
{
   ...
   if (UabInstMode == FALSE)
   	 uabSierraPlatformCheck();

    /* Execute Motion State Machine */
    switch(UabMotionState)
    {
        case start_down_holdoff:  /* Sierra Only */
            if (UabSierraStartDownHoldoff > 0)
                UabSierraStartDownHoldoff--;
            else
            {
                if (UabUpDownLeftRightSwitchState == switch_down_right)
                {
                    ...
                    if (UabInstMode)
                      ...
                      UabEndPwm     = SIERRA_SOFT_START_DOWN_END_PWM;
        ...
        case start_up_holdoff:  /* Sierra Only */
            if (UabSierraStartUpHoldoff > 0)
                UabSierraStartUpHoldoff--;
            else
            {
                if (UabUpDownLeftRightSwitchState == switch_up_left)
                {
        ...
        case stopped:  /* Motor is Off and Brake is set on entry */
            if (PlatformRampChangeDelay > 0)
            {
                PlatformRampChangeDelay--;
                return;
            }
                ...
                if ( (faults & (~(FR_UP_LEFT_FAULT | OB_UP_FAULT))) == 0 )
                {
                    if (UabSierraPlatformPosition == SIERRA_PLATFORM_UNFOLDED)
                    {
                        if (UabUpDownLeftRightSwitchState == switch_down_right)
                        {
                            if (UabSierraPlatformMounting == SIERRA_LEFTSIDE_UP)
                            {
                                upRampRaised   = uabLeftsideRampRaise();
                                downRampRaised = uabRightsideRampRaise();
                            }
                            else
                            {
                                upRampRaised   = uabRightsideRampRaise();
                                downRampRaised = uabLeftsideRampRaise();
                            }
                            if ( (upRampRaised   == SIERRA_RAMP_RAISED) &&
                                 (downRampRaised == SIERRA_RAMP_RAISED) )
                            {
                                UabMotionState = start_down_holdoff;
                                UabSierraStartDownHoldoff = 25;
                            }
                        }
                        else
                        {
                ...
                if ( (faults & (~(FR_DOWN_RIGHT_FAULT | OB_DOWN_FAULT))) == 0 )
                {
                    ...
                    else
                    {
                        ...
                        */
                        ...
                        {
                            UabEndPwm   = SIERRA_SLOW_CRUISE_UP_PWM;
            ...
            if (UabUpDownLeftRightSwitchState == switch_down_right)
            {
                if ( (faults & (~(FR_UP_LEFT_FAULT | OB_UP_FAULT))) == 0 )
                {
                    ...
                    else
                    {
                        ...
                        {
                            UabEndPwm     = SIERRA_SLOW_CRUISE_DOWN_PWM;
                     ...
                     uint32_t prev_pio0, uint32_t prev_pio2)
{
    ...
    if ( (pio0      & SIERRA_UPSIDE_RAMP_OBSTR_SWITCH) !=
         (prev_pio0 & SIERRA_UPSIDE_RAMP_OBSTR_SWITCH) )
    {
        eData[0] = UPSIDE_RAMP_OB_SWITCH_NUM;
        if (pio0 & SIERRA_UPSIDE_RAMP_OBSTR_SWITCH)
          eData[1] = 1;    // Upside Ramp Obstr Detected
        else
          eData[1] = 0;    // Upside Ramp Obstr Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }

    if ( (pio0      & SIERRA_DOWNSIDE_RAMP_OBSTR_SWITCH) !=
         (prev_pio0 & SIERRA_DOWNSIDE_RAMP_OBSTR_SWITCH) )
    {
        eData[0] = DOWNSIDE_RAMP_OB_SWITCH_NUM;
        if (pio0 & SIERRA_DOWNSIDE_RAMP_OBSTR_SWITCH)
          eData[1] = 1;    // Downside Ramp Obstr Detected
        else
          eData[1] = 0;    // Downside Ramp Obstr Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }
