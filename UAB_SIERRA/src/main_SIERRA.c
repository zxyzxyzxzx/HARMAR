/*
******************************************************************************
** Filename:  main_SIERRA.c
**
** Description:
**       This is the UAB main program for the SIERRA system.
...
void uabCommInit( void )
{
    ...
    initData[1] = UAB_SIERRA_MAJOR_VERSION;
    initData[2] = UAB_SIERRA_MINOR_VERSION;
    initData[3] = 0;
    uabInfoPageGet(buff);
...
int main(void)
{
    ...
    UabSystemId = UAB_SYSTEM_ID_SIERRA;
    ...
    */
    ...
    /* Initialize UART0 as DiagDUI Comm port */
    UARTInit(UART0, UART_115200_BAUD);  /* UART0 Intr Enabled after this call */

    /* Initialize UART1 as Ramp Board Comm port */
    UARTInit(UART1, UART_9600_BAUD);    /* UART1 Intr Enabled after this call */
    ...
    uabInfoPageInit(UabSystemId,
                    UAB_SIERRA_MAJOR_VERSION,
                    UAB_SIERRA_MINOR_VERSION);
    ...
    if (UabInstMode)
    {
    	/* Dummy values so Motion State Machine runs during Install */
    ...
    {
    	if (uabSierraRampBoardInit() != OK)
          uabMajorFaultHandler(UAB_MAJOR_FAULT_RAMP_BOARD_INIT);
          /* will not return */
        
    	uabSierraPlatformPositionInit();
    }
    ...
    */
    if ( (pio0 & FOOTREST_UP_LEFT_SWITCH)    &&
         ...
         (pio0 & SEAT_SWITCH) &&
         (((pio0 & INST_DOWN_SWITCH) == 0) ||
          ((pio2 & INST_UP_SWITCH)   == 0)) )
    {   /* Enter IR Learning Mode */
        UabIr0UnitNum = IR_UNIT_UNKNOWN;
        UabIr1UnitNum = IR_UNIT_UNKNOWN;

        /* For Pinnacle/Sierra, user must release the Inst Switch */
        do
        {
           (void)uabPioGet(&pio0, &pio1, &pio2);
           wdtFeed(TRUE);  /* punch WDT */
        }
        while ( ((pio0 & INST_DOWN_SWITCH) == 0) ||
                ((pio2 & INST_UP_SWITCH)   == 0) );
        ...
        if (UabCommEnabled)
        {
            ...
            else
            {
                ...
                */
                ...
                {
                  UpdateDelay = 200;  /* 200 x 10ms = 2 Sec */
