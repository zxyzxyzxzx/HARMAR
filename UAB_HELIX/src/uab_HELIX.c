/*
******************************************************************************
** Filename:  uab_HELIX.c
**
** Description:
**       This file contains all of the functions for the monitoring/control
**       of the HELIX stair lift system via the UAB module.
...
uint8_t  UabRfLeftRightState;
uint8_t  UabInitComplete = FALSE;
uint8_t  UabCommEnabled  = FALSE;
uint8_t  UabTrackOrientation;
uint8_t  UabLed = 0;
uint8_t  SeqNum = 1;

/* HELIX Global Variables */
uint8_t  UabChargeStripDet      = FALSE;
uint8_t  UabChargeStripStopped  = TRUE;    //TRUE means stopped on strip movement now OK
uint8_t  UabChargeStripStopFlag = FALSE;   //FALSE means movement allowed
int32_t  UabChargeStripStopCnt  = 0;
int32_t  UabChargeStripTimeout  = 0;
int32_t  UabSlowStartCnt        = 0;
uint8_t  UabChargeLed           = FALSE;
...
void uabInstModeCheck(uint32_t  pio0)
{
    ...
    pio1 = (uint32_t)(LPC_GPIO1->PIN);
    if ( (pio1 & HELIX_SEAT_SWITCH) &&
         (pio0 & KEY_SWITCH) &&
         (pio0 & RIGHT_SWITCH) &&
         (pio0 & LEFT_SWITCH) )
    {
...
#define HELIX_CURR_SENSE_AMPS_MAX  25.0f
...
void uabSystemOverloadCheck(void)
{
         ...
         (UabMotionState != cruise_down_right) )
    {
        ...
        if ( ((UabUpDownLeftRightSwitchState == switch_up_left) ||
             (UabUpDownLeftRightSwitchState == switch_down_right)) &&
             (UabPwm > 25) )
        {
            curr_sense_amps =
                 ((HELIX_CURR_SENSE_AMPS_MAX * (float)(UabPwm))/100) + 0.75;
...
{
    /* No Runaway on HELIX at the moment */
...
** Function:     uabDiagLed
**
** Description:  This function turns Diag LED  RED, GREEN, AMBER, or OFF.
**               The Charge LED and Power LED are also turned ON/OFF.
**
** Input  Parms: led_color
**
** Output Parms: None
**
** Return Value: None
...
{
    uint16_t ledshift = 0x00;
    uint16_t ledbitval = 0x00;
    uint8_t  loopcnt = 0;
    ...
    ledshift |= 0x02; // add power led

    if (UabChargeLed != 0)
        ledshift |= 0x01;  //add charge led

    ledbitval = ledshift & 0x01;
    // shift out 8 bits on pio0_28 (green) with clock on pio0_29 (red)
    for ( loopcnt = 0; loopcnt <=7; loopcnt++)
    {
        if (ledbitval == 0)
        {
            LPC_GPIO0->OUT &= ~(GREEN_LED_BIT);  //write 0
        }
        else
        {
            LPC_GPIO0->OUT |= GREEN_LED_BIT;  //write 1

        }
        LPC_GPIO0->OUT &= ~(RED_LED_BIT);   //clk down
        LPC_GPIO0->OUT |= RED_LED_BIT;      //clk up 0
        ledshift >>= 1;
        ledbitval = ledshift & 0x01;
    }
}
...
void uabChargeCheck(void)
{
    ...
    */
    ...
    {
        /* If the carriage is moving, check if is on a charge strip, then
        ** disable charge circuit and cause return.
        */
        if (UabMotionState != stopped)//moving
        {
            uabChargerEnable();
            delayUsec(20);
            ...
            if (charge_volts < OFFCHARGE_VOLTS_MIN)//of strip and moving
            {
                if (UabChargeStripStopCnt)//count period before checking
                {
                    UabChargeStripStopCnt--;
                }
                else //count over, now allow check for next strip contact
                {
                    UabChargeStripStopped = FALSE;
                }

            }
            else//moving and on strip
            {
                UabChargeStripStopCnt = 200; //set count to be off strip 2 sec before new cycle
                UabChargeStripTimeout = 300; //set timeout count to be stopped for 3 sec.
                if (UabChargeStripStopped != TRUE)//if not OK to move
                {
                    UabChargeStripStopFlag = TRUE; //signal to stop
                }
            }
        }
        else //stopped
        {
            UabChargeStripStopped = TRUE;//if not moving make sure we are marked stopped.
            if (UabChargeStripTimeout)//if still in timeout period change it back to not stopped.
            {
                UabChargeStripStopped = FALSE;
                UabChargeStripTimeout--;
            }
            else//after timeout, mark stopped and allow movement.
            {
                UabChargeStripStopped = TRUE;
                UabChargeStripStopFlag = FALSE;
            }
            if (UabUpDownReleased)//on switch release(on strip)set OK to move and clear TO.
            {
                UabChargeStripStopped = TRUE;
                UabChargeStripStopFlag = FALSE;
                UabChargeStripTimeout = 0;
            }
        }
    }
                                                            ...
                                                            uint32_t pio2)
{
    ...
    left  = (pio2 & INST_LEFT_SWITCH);//down
    right = (pio0 & INST_RIGHT_SWITCH);//up
...
#define LR_DELAY        100   /*  100 x 10ms =  1 Sec */
#define RF_LR_DELAY     1000   /* 1000 x 10-20ms = ~15 Sec */
...
uabUpDownLeftRightSwitchState_t uabUpDownLeftRightSwitchStateGet(uint32_t pio0)
{
    ...
    static uint16_t rf_delay    = 0;
    static uint16_t left_delay  = 0;
    static uint16_t right_delay = 0;

    /* Read the Armrest LEFT & RIGHT switchs. */
    left  = pio0 & LEFT_SWITCH;
    right = pio0 & RIGHT_SWITCH;

    /* Read the RF LEFT & RIGHT switchs. */
    rf_left  = pio0 & RF_LEFT_SWITCH;
    rf_right = pio0 & RF_RIGHT_SWITCH;
    ...
    if ( (right == 0) && (left) )
    {
        ...
        left_delay = LR_DELAY;
        rf_delay   = RF_LR_DELAY;
        if (right_delay > 0)
        {
            right_delay--;
            UabUpDownState      = switch_off;
            UabRfLeftRightState = switch_off;
            return(switch_off);
        }
        else
        {
        ...
        else
        {
            if (rf_left == 0)//if up
            {
                ...
                if (left_delay > 0)
                 ...
                 {
                     UabUpDownState      = switch_off;
                     // make translation for fob left button up and right button down
                     if (UabTrackOrientation == UAB_LEFT_IS_UP)
                     {
                         UabRfLeftRightState = switch_up_left;//go up left
                         return(switch_up_left);
                     }
                     else
                     {
                         UabRfLeftRightState = switch_down_right;//go up right
                         return(switch_down_right);
                     }
                 }
            }

            if (rf_right == 0)  //if down
            {
                stop_time  = STOP_TIME_MAX;
                left_delay = LR_DELAY/2;
                if (right_delay > 0)
                {
                    right_delay--;
                    UabUpDownState      = switch_off;
                ...
                else
                {
                    ...
                    if (UabTrackOrientation == UAB_LEFT_IS_UP)
                    {
                    UabRfLeftRightState = switch_down_right;//go down right
                    return(switch_down_right);
                    }
                    else
                    {
                        UabRfLeftRightState = switch_up_left;//go down left
                        return(switch_up_left);
                    }
                }
            }
...
void uabMotorOff(void)
{
    ...
    if (UabBrakeState == brake_off)
    {
        ...
        delayMs(50,SYS_TICK_BASED, TRUE);//was 200
    }
...
void uabMotorOn(uint32_t direction)
{
    switch (direction)
    {
        case UAB_UP_LEFT:
            ...
            LPC_CT32B0->MR[2] = (UabPwm * 52);      /* BLI PWM */
        ...
        case UAB_DOWN_RIGHT:
            ...
            /* 24,000 KHz/4Khz = 6000 x Dur/100 = Dur * 60 */
            LPC_CT32B0->MR[0] = (UabPwm * 52);      /* ALI PWM */
    ...
    if (UabBrakeState == brake_on)
    {
        delayMs(15, SYS_TICK_BASED, TRUE);//was 50
...
void uabPwmInit(void)
{
    ...
    LPC_CT32B0->MR[3] = 5200-1; /* Period = 24M/4K = 6000 - 1 = 5999 */
...
void uabPwmChange(uint32_t direction, uint32_t duration)
{
    ...
    {
        LPC_CT32B0->MR[0] = (duration * 52);  /* ALI */
    }
    else
    {
        LPC_CT32B0->MR[2] = (duration * 52);  /* BLI */
    }
}
...
int uabPioGet(uint32_t *pio0_ptr,
              ...
              uint32_t *pio2_ptr)
{
    static uint32_t prev_pio0 = HELIX_PIO0_SWITCH_DEFAULT;
    static uint32_t prev_pio1 = HELIX_PIO1_SWITCH_DEFAULT;
    static uint32_t prev_pio2 = HELIX_PIO2_SWITCH_DEFAULT;
    ...
    **   PIO2: 1,2,14,15
    */
    if ( ((*pio0_ptr & HELIX_PIO0_SWITCH_ALL) != prev_pio0) ||
         ((*pio1_ptr & HELIX_PIO1_SWITCH_ALL) != prev_pio1) ||
         ((*pio2_ptr & HELIX_PIO2_SWITCH_ALL) != prev_pio2) )
    {
        prev_pio0 = (*pio0_ptr & HELIX_PIO0_SWITCH_ALL);
        prev_pio1 = (*pio1_ptr & HELIX_PIO1_SWITCH_ALL);
        prev_pio2 = (*pio2_ptr & HELIX_PIO2_SWITCH_ALL);
        return(NOT_OK);
    }
    return(OK);
}
...
uint32_t uabFaultsGet(uint32_t pio0, uint32_t pio2)
{
    ...
    faults = 0;
     ...
     if ( ((pio2 & OBSTR_LEFT_SWITCH) == 0) ||
         ((prev_faults & OB_UP_FAULT) && (!UabUpDownReleased)) )
        faults |= OB_UP_FAULT;

    if ( ((pio2 & OBSTR_RIGHT_SWITCH) == 0) ||
    ...
    /* If in Installation Mode, disregard indicated faults */
    if (UabInstMode)
    {
        faults &= ~(KEY_OFF_FAULT | SEAT_FAULT);
    }

    /* If pushing UP switch, disregard indicated faults */
    if (UabUpDownLeftRightSwitchState == switch_up_left)
    {
        faults &= ~(FR_DOWN_RIGHT_FAULT | OB_DOWN_FAULT);
    }
    else
    {
        /* If pushing DOWN switch, disregard indicated faults */
...
int uabCheckLimit(uint32_t pio0, uint32_t pio2)
{
         ...
         (pio2 & STOP_DOWN_RIGHT_SWITCH) )
    {
        if (UabInstMode == FALSE)
        {
            ...
            uabMajorFaultHandler(UAB_MAJOR_FAULT_CONF_HELIX_LIMIT_SWITCH_DETECTED);
            /* will not return */
        }
    }
  ...
  //          printf("-0");
            ...
            //fall thru on right states (no break).
            //give time on start and stop to get off switch
  ...
  //            printf("+");
   ...
   //           printf("-4");
                if (UabInstMode == FALSE)
                {
                    //If moving right and hit left limit, right must be faulty.
                    uabMajorFaultHandler(UAB_MAJOR_FAULT_CONF_HELIX_LIMIT_SWITCH_DETECTED);
                    /* will not return */
                }
            }
            ...
            //fall thru on left states (no break).
            //give time on start and stop to get off switch
  ...
  //        printf("-5");
    ...
    //          printf("-8");
                if (UabInstMode == FALSE)
                {
                    //If moving left and hit right limit, left must be faulty.
                    uabMajorFaultHandler(UAB_MAJOR_FAULT_CONF_HELIX_LIMIT_SWITCH_DETECTED);
                    /* will not return */
                }
            }
...
**
**               Changed...Now will not return and go to fault handler if limit detected
**               and return left or right switch.
**
** Input  Parms: uint32_t pio0   Current PIO0 port register
**               uint32_t pio2   Current PIO2 port register
**
...
{
    uabCheckLimit(pio0, pio2);  //will not return if limits bad
...
#define SOFT_START_UP_BEG_PWM                25 //default 25
#define SOFT_START_UP_END_PWM                60 //60
#define INST_SOFT_START_UP_END_PWM           50 //50
#define SOFT_START_UP_PWM_STEP                5
#define SOFT_START_UP_PWM_INTERVAL           12

/*  Soft Start DOWN PWM values */
#define SOFT_START_DOWN_BEG_PWM              25 //default 25
#define SOFT_START_DOWN_END_PWM              50 //50
#define INST_SOFT_START_DOWN_END_PWM         50 //50
#define SOFT_START_DOWN_PWM_STEP              5
#define SOFT_START_DOWN_PWM_INTERVAL         12

/*  Cruise UP and Soft Stop UP PWM values */
#define SLOW_CRUISE_UP_PWM                   40
#define SOFT_STOP_UP_PWM_STEP                10
#define SOFT_STOP_UP_PWM_STEP_C               3 //Charge Strip
#define SOFT_STOP_UP_PWM_INTERVAL             4
#define SOFT_STOP_UP_PWM_INTERVAL_A           4
#define SOFT_STOP_UP_PWM_INTERVAL_C           8 //Charge Strip 12

/* Cruise DOWN and Soft Stop DOWN PWM values */
#define SLOW_CRUISE_DOWN_PWM                 15 //default 15
#define SOFT_STOP_DOWN_PWM_STEP               5
#define SOFT_STOP_DOWN_PWM_STEP_C             3 //Charge Strip
#define SOFT_STOP_DOWN_PWM_INTERVAL           4
#define SOFT_STOP_DOWN_PWM_INTERVAL_A         4
#define SOFT_STOP_DOWN_PWM_INTERVAL_B         2
#define SOFT_STOP_DOWN_PWM_INTERVAL_C         7 //Charge Strip 10

void uabMotionSet(uint32_t pio0, uint32_t pio2)
{
    uint32_t faults;
    int top_bottom;
    static uabUpDownLeftRightSwitchState_t prevUabRfLeftRightState = switch_off;
    uint8_t eData[2];
    ...
    ** the RF Remote Control pushbutton state.
    ** If in Installation mode, get the LEFT/RIGHT state of the
    ** Installation switch.
    */
    ...
    else
    {
        ...
        /* Adjust RF Left/Right state based on HELIX Track orientation */
        if ( (UabUpDownState == switch_off) &&
             (UabRfLeftRightState != prevUabRfLeftRightState) )
        {
            if (UabTrackOrientation == UAB_LEFT_IS_UP)
            {

                eData[0] = RF_LEFT_SWITCH_NUM;
                eData[1] = 0;
                if (UabRfLeftRightState == switch_up_left)
                    eData[1] = 1;
                uabEvent(UAB_SWITCH_CHANGE_EVENT,
                         &eData, sizeof(eData) );

                eData[0] = RF_RIGHT_SWITCH_NUM;
                eData[1] = 0;
                if (UabRfLeftRightState == switch_down_right)
                    eData[1] = 1;
                uabEvent(UAB_SWITCH_CHANGE_EVENT,
                         &eData, sizeof(eData) );

                prevUabRfLeftRightState = UabRfLeftRightState;
            }
            else
            {
                eData[0] = RF_RIGHT_SWITCH_NUM;
 ...
 //   printf("-%d ", UabMotionState);
 //   printf("_%d",UabUpDownLeftRightSwitchState);
    ...
    switch(UabMotionState)
    {
        case stopped:  /* Motor is Off and Brake is set on entry */
            ...
            ** Footrest up/LEFT or Obstruction uP/left, then down movement
            ** is allowed. Check if user is pressing right switch on armrest
            ** or on rf Remote Control. If so, adjust for "soft start"
            ...
            */
            ...
            if ( top_bottom == UAB_TOP_LEFT )
            {
                if ( (faults & (~(FR_UP_LEFT_FAULT | OB_UP_FAULT))) == 0 )
                {
                    if (UabUpDownLeftRightSwitchState == switch_down_right)
                    {
                        if (UabTrackOrientation == UAB_LEFT_IS_UP)//harmar right
                        {
                            UabBegPwm       = SOFT_START_DOWN_BEG_PWM;
                            if (UabInstMode)
                              UabEndPwm     = INST_SOFT_START_DOWN_END_PWM;
                            else
                              UabEndPwm     = SOFT_START_DOWN_END_PWM;
            ...
            else
            {
                ...
                ** If there are no other safety faults other than
                ** Footrest down/right or Obstruction down/right, then
                ** up movement is allowed. Check if user is pressing
                ** LEFT switch on armrest or on RF Remote Control.
                ...
                */
                ...
                {
                    /*
                    ** In mid-track and user is pressing the RIGHT switch
                    ** on armrest or on RF Remote Control, and there are no
                    ** other safety faults other than Footrest up/left or
                    ** Obstruction up/left, then down movement is allowed.
                    ...
                    */
                    if (UabUpDownLeftRightSwitchState == switch_down_right)
                    {
                        if ( (faults & (~(FR_UP_LEFT_FAULT | OB_UP_FAULT))) == 0 )
                        {
                            if (UabChargeStripStopFlag == FALSE) //OK to move
                            {
                                if (UabTrackOrientation == UAB_LEFT_IS_UP)
                                {
                                    UabBegPwm        = SOFT_START_DOWN_BEG_PWM;
                    ...
                    {
                        /*
                        ** In mid-track and user is pressing LEFT switch
                        ** on armrest or on RF Remote Control,and there are no
                        ** other safety faults other than Footrest down/right
                        ** or Obstruction down/right, then up movement is allowed.
                        ** Adjust for "soft start" PWM rate, turn on motor,
                        ** release brake, and transition state machine to
            ...
            */
            if ( (faults & (~(FR_DOWN_RIGHT_FAULT | OB_DOWN_FAULT))) == 0 )
            {
                if (UabUpDownLeftRightSwitchState == switch_up_left)
                {
                    ...
                    {
                        if (UabChargeStripStopFlag == TRUE)
                        {
                            if (UabTrackOrientation == UAB_LEFT_IS_UP)
                            {
                                UabEndPwm       = SLOW_CRUISE_UP_PWM;
                                UabPwmStep      = SOFT_STOP_UP_PWM_STEP_C;
                                UabRampInterval = SOFT_STOP_UP_PWM_INTERVAL_C;
                            }
                            else
                            {
                                UabEndPwm       = SLOW_CRUISE_DOWN_PWM;
                                UabPwmStep      = SOFT_STOP_DOWN_PWM_STEP_C;
                                UabRampInterval = SOFT_STOP_DOWN_PWM_INTERVAL_C;
                            }
                            UabMotionState = soft_stop_up_left;
                        }
                        else
                        {
                            ...
                            if (UabInstMode)
                            {
                              UabPwm = INST_SOFT_START_UP_END_PWM;
                            }
                            else
                            {
                                /* Using Curr Sense Speed Cont (CSSC) to change UabPwm */
                                if (UabTrackOrientation == UAB_LEFT_IS_UP)
                                {
                                    uabPwmAdjust(UAB_DOWN_RIGHT);
                                }
                                else
                                {
                                uabPwmAdjust(UAB_UP_LEFT);
                                }
                            }
                        }
                    }
                ...
                if (UabUpDownLeftRightSwitchState == switch_down_right)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (UabInstMode)
                            {
                                UabPwm = INST_SOFT_START_DOWN_END_PWM;
                            }
                            else
                            {
                                /* Using Curr Sense Speed Ctl (CSSC) to change UabPwm */
                                if (UabTrackOrientation == UAB_LEFT_IS_UP)
                                {
                                    uabPwmAdjust(UAB_UP_LEFT);
                                }
...
**               uint32_t pio1       Current  PIO1 port contents
**               uint32_t pio2       Current  PIO2 port contents
**               uint32_t prev_pio0  Previous PIO0 port contents
**               uint32_t prev_pio1  Previous PIO1 port contents
                         ...
                         uint32_t prev_pio2)
{
    ...
    if ( (pio2      & OBSTR_LEFT_SWITCH) !=
         (prev_pio2 & OBSTR_LEFT_SWITCH) )
    {
        eData[0] = OB_UP_SWITCH_NUM; 
        if ((pio2 & OBSTR_LEFT_SWITCH) == 0)
          eData[1] = 1;         //OBSTR LEFT Detected
        else
          eData[1] = 0;         //OBSTR LEFT Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }

    if ( (pio2      & OBSTR_RIGHT_SWITCH) !=
         (prev_pio2 & OBSTR_RIGHT_SWITCH) )
    {
        eData[0] = OB_DOWN_SWITCH_NUM; 
        if ((pio2 & OBSTR_RIGHT_SWITCH) == 0)
          eData[1] = 1;         //OBSTR RIGHT Detected
        else
          eData[1] = 0;         //OBSTR RIGHT Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }
         ...
         (prev_pio0 & STOP_UP_LEFT_SWITCH) )
    {
        ...
        if (pio0 & STOP_UP_LEFT_SWITCH)
          eData[1] = 1;         //STOP LEFT Detected
        else
          eData[1] = 0;         //STOP LEFT Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }
         ...
         (prev_pio2 & STOP_DOWN_RIGHT_SWITCH) )
    {
        ...
        if (pio2 & STOP_DOWN_RIGHT_SWITCH)
          eData[1] = 1;         //STOP RIGHT Detected
        else
          eData[1] = 0;         //STOP RIGHT Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }
         ...
         (prev_pio0 & FOOTREST_UP_LEFT_SWITCH) )
    {
        ...
        if (pio0 & FOOTREST_UP_LEFT_SWITCH)
          eData[1] = 1;         //FR LEFT Detected
        else
          eData[1] = 0;         //FR LEFT Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }
         ...
         (prev_pio0 & FOOTREST_DOWN_RIGHT_SWITCH) )
    {
        ...
        if (pio0 & FOOTREST_DOWN_RIGHT_SWITCH)
          eData[1] = 1;         //FR RIGHT Detected
        else
          eData[1] = 0;         //FR RIGHT Off
        uabEvent(UAB_SWITCH_CHANGE_EVENT,
                  &eData, sizeof(eData) );
    }

    if ( (pio0      & LEFT_SWITCH) !=
         (prev_pio0 & LEFT_SWITCH) )
    {
    ...
    if ( (pio0      & RIGHT_SWITCH) !=
         (prev_pio0 & RIGHT_SWITCH) )
    {
    ...
    if (UabInstMode)
    {
        if ( (pio0      & INST_RIGHT_SWITCH) !=
             (prev_pio0 & INST_RIGHT_SWITCH) )
        {
            eData[0] = INST_DOWN_SWITCH_NUM;
            if (pio0 & INST_RIGHT_SWITCH)
              eData[1] = 0;         //INST RIGHT Off
            else
              eData[1] = 1;         //INST RIGHT On
            uabEvent(UAB_SWITCH_CHANGE_EVENT,
                      &eData, sizeof(eData) );
        }

        if ( (pio2      & INST_LEFT_SWITCH) !=
             (prev_pio2 & INST_LEFT_SWITCH) )
        {
            eData[0] = INST_UP_SWITCH_NUM;
            if (pio2 & INST_LEFT_SWITCH)
              eData[1] = 0;         //INST LEFT Off
            else
              eData[1] = 1;         //INST LEFT On
            uabEvent(UAB_SWITCH_CHANGE_EVENT,
                      &eData, sizeof(eData) );
        }
    }
