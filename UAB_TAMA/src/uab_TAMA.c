...
**       of the TAMA stair lift system via the UAB module.
...
**               when:
**                      Current Sense is > 25.0 Amps (scaled)
**
...
void uabSystemOverloadCheck(void)
{
         ...
         (UabMotionState != cruise_down_right) )
    {
        ...
        {
            curr_sense_amps =
                 ((CURR_SENSE_AMPS_MAX * (float)(UabPwm))/100) + 0.75;
...
**
** Description:  No Runaway on TAMA**
** Input  Parms: None
** Output Parms: None
**
** Global Vars:
...
**
**               During normal operation, the charger voltage and battery
...
uabUpDownLeftRightSwitchState_t uabUpDownLeftRightSwitchStateGet(uint32_t pio0)
{
    ...
    */
    up_left    = pio0 & LEFT_SWITCH;
    down_right = pio0 & RIGHT_SWITCH;
...
**               For Pinnacle/Sierra, the brake is also engaged.
**
** Input  Parms: None 
**
** Output Parms: None
...
#define FR_UNDER_TIMEOUT      150   /* 150 x 10ms = 1.5 Sec */

#define NO_DIR     0
#define LEFT_DIR   1
#define RIGHT_DIR  2

uint8_t DirAtFootrestUnderFault = NO_DIR;
...
uint32_t uabFaultsGet(uint32_t pio0, uint32_t pio2)
{
    ...
    uint32_t        bit;
    int             i;
    static uint8_t  prevDirAtFootrestUnderFault = NO_DIR;
    static uint8_t  movementCntr = 0;
    int             top_bottom;

    top_bottom = uabCheckTopBottom(pio0, pio2);
    if ( ((top_bottom == UAB_TOP_LEFT) ||
          (top_bottom == UAB_BOTTOM_RIGHT)) &&
         (pio0 & FOOTREST_UNDER_SWITCH) )   
    {
        faults |= FR_UNDER_FAULT;

        DirAtFootrestUnderFault     = NO_DIR;
        prevDirAtFootrestUnderFault = NO_DIR;
        movementCntr  = 0;
        goto skip1;
    }

    if ( ((pio0 & FOOTREST_UNDER_SWITCH) == 0) &&
         (((prevDirAtFootrestUnderFault == LEFT_DIR) &&
           (UabUpDownLeftRightSwitchState == switch_down_right)) ||
          ((prevDirAtFootrestUnderFault == RIGHT_DIR) &&
           (UabUpDownLeftRightSwitchState == switch_up_left))) )
    {
        DirAtFootrestUnderFault     = NO_DIR;
        prevDirAtFootrestUnderFault = NO_DIR;
        movementCntr  = 0;
         ...
         ((prev_faults & FR_UNDER_FAULT) && (!UabUpDownReleased)) )
    {
        ...
        if (UabUpDownLeftRightSwitchState == switch_up_left)
        {
            if (prevDirAtFootrestUnderFault == NO_DIR)
              DirAtFootrestUnderFault = LEFT_DIR;
        }
        else if (UabUpDownLeftRightSwitchState == switch_down_right)
        {
            if (prevDirAtFootrestUnderFault == NO_DIR)
              DirAtFootrestUnderFault = RIGHT_DIR;
        }

        prevDirAtFootrestUnderFault = DirAtFootrestUnderFault;

        if (DirAtFootrestUnderFault == LEFT_DIR)
        {
            if (UabUpDownLeftRightSwitchState == switch_down_right)
            {
                if (++movementCntr > FR_UNDER_TIMEOUT)
                  uabMajorFaultHandler(UAB_MAJOR_FAULT_TAMA_FRUNDER);
            }
            else
            {
                movementCntr  = 0;
        ...
        else if (DirAtFootrestUnderFault == RIGHT_DIR)
        {
            if (UabUpDownLeftRightSwitchState == switch_up_left)
            {
                if (++movementCntr > FR_UNDER_TIMEOUT)
...
#define SOFT_START_UP_BEG_PWM                25
#define SOFT_START_UP_END_PWM                60
#define SOFT_START_UP_PWM_STEP                5
#define SOFT_START_UP_PWM_INTERVAL           12

/*  Soft Start DOWN PWM values */
#define SOFT_START_DOWN_BEG_PWM              25
#define SOFT_START_DOWN_END_PWM              50
...
#define SLOW_CRUISE_DOWN_PWM                 15
            ...
            if (UabUpDownLeftRightSwitchState == switch_up_left)
            {
                     ...
                     (DirAtFootrestUnderFault == RIGHT_DIR) )
                {
                    if ( uabCheckTopBottom(pio0, pio2) == UAB_TOP_LEFT )
                    {
                        ...
                        ** motor, engage the brake, and transition motion
                 ...
                 (DirAtFootrestUnderFault == LEFT_DIR) )
            {
                if (UabUpDownLeftRightSwitchState == switch_down_right)
                {
                    ...
                    else
                    {
                        ...
                        {
                            /* Using Curr Sense Speed Ctrl (CSSC) to change UabPwm */
...
**
** Description:  If debug mode is enabled, this function displays any change
**               in any switch connected to the LPC122x PIO pins.
**
